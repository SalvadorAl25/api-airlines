...
public class BoardingPassService {
    ...
    private List<Integer> availableSeats(SeatEntity seat,Integer passNotSet){
        ...
        if (seat.getAirplane().getAirplaneId() == 1) {
                ...
                if (seat.getSeatColumn().equals("C")){
                    ...
                    do {
                        for (int col = 2; col > -1; col--) {
                            oneSeat = seatService.findSeatByColumnAndRow(col1a1.get(col), seatRow, seat.getAirplane().getAirplaneId());
                    ...
                    do { //realizara la accion de buscar por fila la cantidad de asientos disponibles para los pasajeros faltantes
                        for (int col = 0; col < col1a1.size(); col++) {
                            ...
                            if (seatsIds.size() != passNotSet){
                                if(!seatsNotAvailable.contains(oneSeat)) {
                                    if (!findIfTheSeatIsFree(oneSeat, seat.getAirplane().getAirplaneId()) && oneSeat == seat.getSeatId() && col == 0)
                                        occupied1 = true;
                                    else if (findIfTheSeatIsFree(oneSeat, seat.getAirplane().getAirplaneId()) && oneSeat != seat.getSeatId() && col == 0)
                                        seatsIds.add(oneSeat);
                                    if (!findIfTheSeatIsFree(oneSeat, seat.getAirplane().getAirplaneId()) && oneSeat != seat.getSeatId() && col == 1)
                                        occupied2 = true;
                                    else if (findIfTheSeatIsFree(oneSeat, seat.getAirplane().getAirplaneId()) && oneSeat != seat.getSeatId() && col == 1)
                                        seatsIds.add(oneSeat);
                                    if (occupied1 && occupied2) {
                                        ...
                                        if (findIfTheSeatIsFree(oneSeat, seat.getAirplane().getAirplaneId()) && col == 2)
                                            seatsIds.add(oneSeat);
                                    }
                                }
                            }
                ...
                if (!seat.getSeatColumn().equals("G")) {
                    do {
                        for (int col = 0; col < col2a1.size(); col++) {
                            oneSeat = seatService.findSeatByColumnAndRow(col2a1.get(col), seatRow, seat.getAirplane().getAirplaneId());
                            if (seatsIds.size() != passNotSet){
                                if(!seatsNotAvailable.contains(oneSeat)) {
                                    if (!findIfTheSeatIsFree(oneSeat, seat.getAirplane().getAirplaneId()) && oneSeat != seat.getSeatId() && col == 0)
                                        occupied1 = true;
                                    ...
                                    if (!findIfTheSeatIsFree(oneSeat, seat.getAirplane().getAirplaneId()) && oneSeat == seat.getSeatId() && col == 1)
                                        ...
                                        seatsIds.add(oneSeat);
                                    if (occupied1 && occupied2) {
                                        if (findIfTheSeatIsFree(oneSeat, seat.getAirplane().getAirplaneId()) && oneSeat != seat.getSeatId() && col == 2) {
                                            seatsIds.add(oneSeat);
                                        }
                                    } else if (occupied2)
                                        if (findIfTheSeatIsFree(oneSeat, seat.getAirplane().getAirplaneId()) && oneSeat != seat.getSeatId() && col == 2) {
        ...
        if (seat.getAirplane().getAirplaneId() == 2) {
            if (col1a2.contains(seat.getSeatColumn())) {
                do {
                    for (int col = 0; col < col1a2.size(); col++) {
                        oneSeat = seatService.findSeatByColumnAndRow(col2a1.get(col), seatRow, seat.getAirplane().getAirplaneId());
                        if (seatsIds.size() != passNotSet){
                            if(!seatsNotAvailable.contains(oneSeat))
                                if (findIfTheSeatIsFree(oneSeat, seat.getAirplane().getAirplaneId()) && oneSeat != seat.getSeatId())
            ...
            if (col2a2.contains(seat.getSeatColumn())){
                do {
                    for (int col = 0; col < col2a2.size(); col++) {
                        oneSeat = seatService.findSeatByColumnAndRow(col2a1.get(col), seatRow, seat.getAirplane().getAirplaneId());
                        if (seatsIds.size() != passNotSet){
                            if(!seatsNotAvailable.contains(oneSeat))
                                if (findIfTheSeatIsFree(oneSeat, seat.getAirplane().getAirplaneId()) && oneSeat != seat.getSeatId())
            ...
            if (col3a2.contains(seat.getSeatColumn())){
                do {
                    for (int col = 0; col < col3a2.size(); col++) {
                        oneSeat = seatService.findSeatByColumnAndRow(col2a1.get(col), seatRow, seat.getAirplane().getAirplaneId());
                        if (seatsIds.size() != passNotSet){
                            if(!seatsNotAvailable.contains(oneSeat))
                                if (findIfTheSeatIsFree(oneSeat, seat.getAirplane().getAirplaneId()) && oneSeat != seat.getSeatId())
    ...
    private List<Integer> availableSeats2(Integer seatTypeId, Integer passNotSeat, Integer lastId){
        Optional<SeatEntity> seatEntity = seatService.findById(lastId);
        Integer airplaneId = seatEntity.get().getAirplane().getAirplaneId();
    ...
    private List<ManyPassengersEntity> findSeatToAssign(List<ManyPassengersEntity> groupPassengers, Integer lastId){
        int seatIdByGroup = 0;
        int passNotSeat = 0;
        List<Integer> seatsIds = new ArrayList<>();
        ...
        if (passNotSeat!=0 && seatIdByGroup != 0) {
            seatsIds = availableSeats(seatEntity.get(), passNotSeat);
            seatsNotAvailable.add(seatIdByGroup);
            seatsNotAvailable.addAll(seatsIds);
        }
        else {
            seatsIds = availableSeats2(groupPassengers.get(0).getSeatTypeId(), passNotSeat, lastId);
        }
    ...
    private List<ManyPassengersEntity> setSeatsByGroup(List<ManyPassengersEntity> manyPassengers){
        ...
        for (int i = 0; i<manyPassengers.size(); i++ ){
            if (i != 0){ // verifica si es parte del grupo del anterior pasajero
                if (manyPassengers.get(i).getPurchaseId() == lastId){
                    ...
                    passGroupWithSeat.addAll(findSeatToAssign(groupPassengers, lastId));
    ...
    private List<ManyPassengersEntity> findAllPassengers(List<BoardingPassEntity> boardings){
        ...
        manyPassengers = setSeatsByGroup(manyPassengers);
